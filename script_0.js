...
Continuar?`);
  ...
  window.open(base + encodeURIComponent(text), "_blank");
    ...
    function buildMiniListPayloadFromCurrent(){
      const playersAll = Object.values(state.players||{});
      const playersArr = playersAll.filter(p => !(p && p.inactive));
      const present = playersArr
        .filter(p => isPresent(p.id))
        .sort((a,b)=>{
          const orderA = teamOf(a.id) === 1 ? 0 : teamOf(a.id) === 2 ? 1 : 2;
          const orderB = teamOf(b.id) === 1 ? 0 : teamOf(b.id) === 2 ? 1 : 2;
          if(orderA !== orderB) return orderA - orderB;
          const checkA = Number((state.attendance && state.attendance[a.id] && state.attendance[a.id].checkedInAtMs) || 0);
          const checkB = Number((state.attendance && state.attendance[b.id] && state.attendance[b.id].checkedInAtMs) || 0);
          if(checkA !== checkB) return checkA - checkB;
          return String(a.name || '').localeCompare(String(b.name || ''), 'pt-BR');
        });

      const rows = present.map((p, idx)=>( {
        number: idx + 1,
        name: String(p.name || '').trim(),
        team: teamOf(p.id),
        teamLabel: miniListTeamLabel(teamOf(p.id))
      }));

      const countTeam1 = rows.filter(r => r.team === 1).length;
      const countTeam2 = rows.filter(r => r.team === 2).length;
      const countWaiting = rows.filter(r => r.team == null).length;
    ...
    function buildMiniListExportElement(payload){
      const wrap = document.createElement('div');
      const team1Label = String(state.team1Name || 'Time 1').trim() || 'Time 1';
      const team2Label = String(state.team2Name || 'Time 2').trim() || 'Time 2';
      const rows = Array.isArray(payload.rows) ? payload.rows : [];
      const team1Rows = rows.filter(r => r.team === 1);
      const team2Rows = rows.filter(r => r.team === 2);
      const waitingRows = rows.filter(r => r.team == null);
      const schedule = getMatchScheduleMeta();
      const infoParts = [];
      if(state.matchLocation) infoParts.push(`${escapeHtml(state.matchLocation)}`);
      if(state.matchDate) infoParts.push(`${escapeHtml(state.matchDate.split('-').reverse().join('/'))}`);
      if(state.matchTime) infoParts.push(`${escapeHtml(state.matchTime.replace(':','h'))}`);

      wrap.style.width = '1080px';
      wrap.style.minHeight = '1080px';
      wrap.style.background = 'linear-gradient(180deg,#0b2a8d 0%, #102c72 26%, #14398d 100%)';
      wrap.style.padding = '28px';
      wrap.style.borderRadius = '42px';
      wrap.style.fontFamily = 'system-ui,-apple-system,Segoe UI,Roboto,Arial';
      wrap.style.color = '#ffffff';
      wrap.style.boxSizing = 'border-box';
      wrap.style.border = '12px solid #ffffff';
      wrap.style.position = 'relative';
      wrap.style.overflow = 'hidden';

      function buildRows(list){
        return list.map((row)=>`
          <div style="display:grid;grid-template-columns:64px 1fr;align-items:center;min-height:54px;border-bottom:1px solid #dbe3f1;">
            <div style="display:flex;align-items:center;justify-content:center;height:100%;background:#facc15;color:#0b2a8d;font-size:22px;font-weight:1000;border-right:1px solid #dbe3f1;">${escapeHtml(String(row.number || ''))}</div>
            <div style="padding:0 14px;font-size:21px;font-weight:900;color:#11206f;line-height:1.05;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${escapeHtml(row.name || '')}</div>
          </div>
        `).join('') || `<div style="padding:16px;font-size:16px;font-weight:700;color:#64748b;">Sem jogadores</div>`;
      }

      wrap.innerHTML = `
        <div style="position:absolute;inset:0;background:
          radial-gradient(circle at 8% 10%, rgba(250,204,21,0.22), transparent 18%),
          radial-gradient(circle at 92% 14%, rgba(250,204,21,0.18), transparent 16%),
          radial-gradient(circle at 50% 100%, rgba(255,255,255,0.06), transparent 35%);
          pointer-events:none;"></div>

        <div style="position:relative;z-index:1;">
          <div style="display:flex;align-items:center;justify-content:space-between;gap:16px;">
            <div style="display:flex;align-items:center;gap:16px;">
              <img src="./icon-512.png" alt="Manchette Volleyball" style="width:94px;height:94px;object-fit:contain;display:block;filter:drop-shadow(0 6px 14px rgba(0,0,0,0.18));">
              <div>
                <div style="display:inline-flex;align-items:center;padding:8px 14px;border-radius:999px;background:#facc15;color:#0b2a8d;font-size:18px;font-weight:1000;letter-spacing:0.04em;text-transform:uppercase;">Figurinha rápida</div>
                <div style="margin-top:10px;font-size:34px;font-weight:1000;line-height:1;color:#ffffff;">Jogadores da rodada</div>
                <div style="margin-top:8px;font-size:18px;font-weight:700;color:#dbeafe;">${infoParts.length ? infoParts.join(' • ') : escapeHtml(payload.code || '')}</div>
              </div>
            </div>
          </div>

          <div style="display:grid;grid-template-columns:1fr 1fr;gap:18px;margin-top:24px;">
            <div style="background:#ffffff;border-radius:26px;overflow:hidden;box-shadow:0 14px 32px rgba(0,0,0,0.18);">
              <div style="background:#facc15;color:#0b2a8d;text-align:center;font-size:30px;font-weight:1000;letter-spacing:0.04em;padding:12px 16px;text-transform:uppercase;">${escapeHtml(team1Label)}</div>
              ${buildRows(team1Rows)}
            </div>
            <div style="background:#ffffff;border-radius:26px;overflow:hidden;box-shadow:0 14px 32px rgba(0,0,0,0.18);">
              <div style="background:#facc15;color:#0b2a8d;text-align:center;font-size:30px;font-weight:1000;letter-spacing:0.04em;padding:12px 16px;text-transform:uppercase;">${escapeHtml(team2Label)}</div>
              ${buildRows(team2Rows)}
            </div>
          </div>

          <div style="margin-top:18px;background:#ffffff;border-radius:22px;padding:14px 18px;box-shadow:0 14px 32px rgba(0,0,0,0.16);display:flex;align-items:flex-start;gap:12px;">
            <div style="padding:8px 12px;border-radius:999px;background:#0b2a8d;color:#facc15;font-size:18px;font-weight:1000;text-transform:uppercase;white-space:nowrap;">Aguardando</div>
            <div style="padding-top:6px;font-size:18px;font-weight:800;line-height:1.35;color:#11206f;">${waitingRows.length ? waitingRows.map(r => `${escapeHtml(String(r.number || ''))} ${escapeHtml(r.name || '')}`).join(' <span style="color:#f59e0b;">•</span> ') : 'Nenhum inscrito aguardando'}</div>
          </div>
        </div>
      `;
      return wrap;
    ...
    async function downloadMiniListPng(){
      if(!session.admin) return alert('Somente o admin pode compartilhar/baixar.');
      if(typeof html2canvas !== 'function') return alert('Biblioteca de exportação não carregou. Recarregue a página.');
      const payload = buildMiniListPayloadFromCurrent();
      if((payload.rows || []).length === 0) return alert('Ainda não há presentes para baixar a mini lista.');

      const temp = buildMiniListExportElement(payload);
      ...
      try{
        await waitForElementImages(temp);
        const canvas = await html2canvas(temp, { scale: 2, backgroundColor: '#f6f7fb', useCORS: true });
        const code = payload.code || 'manchette';
        const stamp = new Date().toISOString().slice(0,10);
        canvas.toBlob((blob)=>{
          if(!blob) return alert('Não consegui gerar a mini lista.');
          const a = document.createElement('a');
          const url = URL.createObjectURL(blob);
          a.href = url;
          a.download = `mini-lista-${code}-${stamp}.png`;
          document.body.appendChild(a);
          a.click();
          a.remove();
          setTimeout(()=>URL.revokeObjectURL(url), 2000);
          setInfo('Mini lista baixada.');
        }, 'image/png');
      }catch(e){
        alert('Falha ao gerar a mini lista: ' + (e && e.message ? e.message : e));
      }finally{
        temp.remove();
      }
    }
